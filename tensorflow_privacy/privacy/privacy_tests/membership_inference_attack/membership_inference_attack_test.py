...
class RunAttacksTest(parameterized.TestCase):
  ...
  def test_run_attack_threshold_calculates_correct_epsilon(
    ...
    np.testing.assert_almost_equal(
        ...
        np.array([0.34695111, 0.34695111, 0.05616349, 0.05616349, 0.02944212]))

  @parameterized.parameters(AttackType.LOGISTIC_REGRESSION,
  ...
  @mock.patch.object(models, 'MultilayerPerceptronAttacker',
                     MockTrainedAttacker)
  ...
  def test_run_attack_trained_calculates_correct_epsilon(self, attack_type):
    result = mia._run_attack(
        AttackInputData(
            logits_train=np.ones((1000, 5)), logits_test=np.zeros((100, 5))),
        attack_type)
    # The prediction would be all 1 for training and all 0 for test.
    expected_bounds = {
        elb.BoundMethod.KATZ_LOG: [
            5.27530977, 2.97796578, -0.00720554, -0.01623037
        ],
        elb.BoundMethod.ADJUSTED_LOG: [
            5.27580935, 2.98292432, -0.00717236, -0.01614769, -7.62368550
        ],
        elb.BoundMethod.BAILEY: [
            5.87410287, 3.57903543, -0.00718316, -0.01625286
        ],
        elb.BoundMethod.INV_SINH: [
            4.95123977, 2.65964282, -0.00720547, -0.01623030
        ],
        elb.BoundMethod.CLOPPER_PEARSON: [5.56738762, 3.31454626],
    }
    self.assertEqual(result.epsilon_lower_bound_value.bounds.keys(),
                     expected_bounds.keys())
    for key in expected_bounds:
      np.testing.assert_almost_equal(
          result.epsilon_lower_bound_value.bounds[key], expected_bounds[key])
    np.testing.assert_almost_equal(
        result.epsilon_lower_bound_value.get_max_epsilon_bounds(),
        np.array([5.56738762, 3.31454626]))
