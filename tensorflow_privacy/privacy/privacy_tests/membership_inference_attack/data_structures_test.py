...
class EpsilonLowerBoundValueTest(parameterized.TestCase):
  ...
  def test_epsilon_lower_bound_value(self):
    bounds = {
        ...
        elb.BoundMethod.BAILEY: np.array([5, 4.]),
  ...
  def test_epsilon_lower_bound_value_one_array_empty(self):
    ...
    np.testing.assert_allclose(elbv.get_max_epsilon_bounds(), [-2, -2.])
...
class SingleMembershipProbabilityResultTest(absltest.TestCase):
  ...
  def test_collect_results(self):
    ...
    self.assertListEqual(summary, [
        ...
        '  with 0.8000 as the threshold on membership probability, the precision-recall pair is (0.8000, 0.8000)',
        '  with 0.7000 as the threshold on membership probability, the precision-recall pair is (0.6250, 1.0000)',
        '  thresholding on membership probability achieved an AUC of 0.94',
        '  thresholding on membership probability achieved an advantage of 0.80',
        '  thresholding on membership probability achieved top-5 epsilon lower bound of 0.3719, 0.2765, 0.1207, 0.1207, -0.0064'
    ])
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.some_attack_result = SingleAttackResult(
        ...
        roc_curve=RocCurve(
            ...
            thresholds=np.array([0, 1, 2]),
            test_train_ratio=1.0),
        epsilon_lower_bound_value=EpsilonLowerBoundValue(bounds={
            elb.BoundMethod.KATZ_LOG: np.array([2, 2.]),
        }),
        data_size=DataSize(ntrain=1, ntest=1))
...
class AttackResultsTest(absltest.TestCase):
        ...
        epsilon_lower_bound_value=EpsilonLowerBoundValue(
            bounds={
                elb.BoundMethod.INV_SINH:
                    np.array([2.65964282, 2.65964282, -0.01648963, -0.01648963]
                            ),
                elb.BoundMethod.CLOPPER_PEARSON:
                    np.array([3.28134635, 3.28134635]),
            }),
        data_size=DataSize(ntrain=1, ntest=1))
        ...
        epsilon_lower_bound_value=EpsilonLowerBoundValue(
            bounds={
                elb.BoundMethod.KATZ_LOG:
                    np.array([-0.01648981, -0.01648981]),
                elb.BoundMethod.ADJUSTED_LOG:
                    np.array([-0.01640757, -0.01640757]),
            }),
        data_size=DataSize(ntrain=1, ntest=1))
  ...
  def test_get_result_with_max_epsilon_first(self):
    results = AttackResults(
        [self.random_classifier_result, self.perfect_classifier_result])
    self.assertEqual(results.get_result_with_max_epsilon(),
                     self.perfect_classifier_result)
  ...
  def test_summary_by_slices(self):
    results = AttackResults(
        [self.perfect_classifier_result, self.random_classifier_result])
    # raise ValueError(results.summary(by_slices=True))
    self.assertSequenceEqual(
        ...
        'True\n'
        '  THRESHOLD_ATTACK (with 1 training and 1 test examples) achieved top-5'
        ' epsilon lower bounds of 3.2813, 3.2813 on slice CORRECTLY_CLASSIFIED=True\n\n'
        ...
        '  THRESHOLD_ATTACK (with 1 training and 1 test examples) achieved top-5'
        ' epsilon lower bounds of 3.2813, 3.2813\n\n'
        ...
        '  THRESHOLD_ATTACK (with 1 training and 1 test examples) achieved top-5'
        ' epsilon lower bounds of -0.0164, -0.0164')
  ...
  def test_calculate_pd_dataframe(self):
    ...
    df_expected = pd.DataFrame({
        ...
        'Epsilon lower bound_5': [np.nan, np.nan],
    })
